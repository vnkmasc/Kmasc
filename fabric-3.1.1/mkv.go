...
// findKeyFile tìm file key ở nhiều vị trí khác nhau
func findKeyFile(filename string) (string, error) {
	// Danh sách các vị trí tìm kiếm
	searchPaths := []string{
		".",                   // Current directory
		"/",                   // Root directory
		"/tmp",                // Temp directory
...
func InitializeKeyManagement(password string) error {
	...
	// Lưu keys vào tất cả các vị trí có thể
	for _, path := range savePaths {
		...
		// Lưu K0
		k0Path := path + "/k0.key"
		err = ioutil.WriteFile(k0Path, k0, 0644)
		if err != nil {
			logToFileMKV("SAVE_K0", "", k0Path, "FAIL", err.Error())
		}

		// Lưu K1 đã mã
		encryptedK1Path := path + "/encrypted_k1.key"
		err = SaveEncryptedK1ToFile(encryptedK1, encryptedK1Path)
		if err != nil {
			logToFileMKV("SAVE_ENCRYPTED_K1", "", encryptedK1Path, "FAIL", err.Error())
			continue // Tiếp tục với vị trí khác
		}

		logToFileMKV("SAVE_KEYS", "", path, "SUCCESS", "")
	}
