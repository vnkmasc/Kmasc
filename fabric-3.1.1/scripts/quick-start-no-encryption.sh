...
# Step 5: Build encryption library (SKIPPED - encryption disabled)
step5_build_encryption() {
    echo "Step 5: Building encryption library..."
    print_status "INFO" "Encryption is disabled - skipping encryption library build"
    print_status "PASS" "encryption library build skipped"
}
...
step8_next_steps() {
    ...
    echo
    echo "🎉 Congratulations! Your Hyperledger Fabric WITHOUT encryption is ready!"
    echo
    echo "📋 What's been set up:"
    echo "   ✅ Environment dependencies (Go, OpenSSL, Docker)"
    echo "   ✅ Encryption DISABLED (for performance testing)"
    echo "   ✅ Fabric binaries without encryption"
    echo "   ✅ Test network running"
    echo
    echo "🔍 To verify encryption is disabled:"
    echo "   docker exec peer0.org1.example.com cat /root/state_encryption_disabled.log"
    echo
