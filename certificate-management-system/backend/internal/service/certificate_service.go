...
import (
	...
	"github.com/tuyenngduc/certificate-management-system/backend/internal/mapper"
...
func generateCertificateHash(cert *models.Certificate, user *models.User, faculty *models.Faculty, university *models.University) string {
	data := fmt.Sprintf("%s|%s|%s|%s|%s|%s|%s|%s|%f|%s",
		...
		user.DateOfBirth.Format("2006-01-02"), // ngày sinh
		cert.StudentCode,                      // mã sv
		user.CitizenIdentificationNumber,      // căn cước công dân
		user.Email,                            // email
